...
export function getAnnularSectorPathAttribute(options) {
...
// Options:
// - degrees: icon's position these angles, clockwise
...
export function getIconPosition(
  ...
  const origin = { x: wrapperSize / 2, y: wrapperSize / 2 };
  var radians = degrees * (Math.PI / 180);
  var radius = innerRadius + (outerRadius - innerRadius) / 2;

  return {
    x: origin.x + Math.cos(radians) * radius - iconSize / 2,
    y: origin.y + Math.sin(radians) * radius - iconSize / 2,
  };
}
